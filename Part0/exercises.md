...
sequenceDiagram
    ...
    Note right of browser: The user clicks the Save button, submitting the form

    browser->>server: POST [https://studies.cs.helsinki.fi/exampleapp/new_note](https://studies.cs.helsinki.fi/exampleapp/new_note)
    activate server
    Note left of server: The server processes the form data and saves the new note
...
sequenceDiagram
    ...
    Note right of browser: The browser's JavaScript event handler executes. It prevents the default form submission, adds the new note to the list locally, and redraws the notes on the screen.
    ...
    activate server
    Note left of server: The server receives the JSON payload containing the new note and saves it.
    server-->>browser: HTTP status code 201 (Created)
    deactivate server

    Note right of browser: The browser stays on the same page. No redirects or further HTTP requests occur.